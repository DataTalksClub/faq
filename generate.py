...
def collect_questions():
        ...
        for question_file in sorted(course_dir.glob('*.md')):
            try:
                ...
                if not frontmatter.get('question'):
                    print(f"Warning: {question_file.name} missing question field - including with placeholder")
                    skipped_files.append(question_file.name)
                    
                    # Include with placeholder instead of skipping
                    question_text = f"Question from {question_file.name}"
                else:
                    question_text = frontmatter['question']
    ...
    # Print summary of files with missing question fields
    if skipped_files:
        print(f"\nSummary: {len(skipped_files)} files had missing question fields but were included with placeholders:")
