...
Kafka allows you to manually control when message offsets are committed.
This is useful when you want to **commit offsets only after successful message processing**, ensuring reliable processing.
...
```
Offsets are committed **only after** the message is successfully processed.
If an error occurs, the offset is not committed, so the message can be consumed again.