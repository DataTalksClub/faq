...
3. Set the start command (e.g. `uvicorn app:app --host 0.0.0.0 --port $PORT`). Render injects `$PORT` at runtime — listen on it, don't hardcode.
4. Free tier sleeps after ~15 min idle, so the first request after that takes ~30s to wake up.

Render is a popular free-tier choice for project deployment because it doesn't require a credit card. Other options: Fly.io, Google Cloud Run (both require a card), Railway, PythonAnywhere.
